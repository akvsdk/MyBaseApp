 ...
 * Description:
 */
public abstract class MyBaseCallBack<T> extends ObjectCallback<T> {
    private ProgressDialog loadMask;
    protected Context context;

    public MyBaseCallBack(Context mtx) {
        this.context = mtx;
    ...
    public MyBaseCallBack(Context mtx, TypeReference<T> typeReference) {
        super(typeReference);
        this.context = mtx;
        loadMask = new ProgressDialog(context);
        loadMask.setProgressStyle(0);
